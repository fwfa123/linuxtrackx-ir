...
PLUGIN_API int XPluginStart(char *outName, char *outSig, char *outDesc) {
  ...
  recenter_cmd =
      XPLMCreateCommand("linuxtrack/ltr_recenter", "Recenter tracking");
  XPLMRegisterCommandHandler(run_cmd, cmd_cbk, true, (void *)TOGGLE);
  XPLMRegisterCommandHandler(start_cmd, cmd_cbk, true, (void *)START);
  XPLMRegisterCommandHandler(stop_cmd, cmd_cbk, true, (void *)STOP);
  XPLMRegisterCommandHandler(pause_cmd, cmd_cbk, true, (void *)PAUSE);
  XPLMRegisterCommandHandler(recenter_cmd, cmd_cbk, true, (void *)RECENTER);
  ...
  head_psi_out = XPLMRegisterDataAccessor(
      ...
      NULL, NULL,             // Float array accessors
      NULL, NULL,             // Raw data accessors
      (void *)&current_head_heading,
      (void *)&current_head_heading); // Refcons not used

  head_the_out =
      XPLMRegisterDataAccessor("linuxtrack/pilots_head_the",
                               xplmType_Float,         // The types we support
                               0,                      // Writable
                               NULL, NULL,             // Integer accessors
                               GetHeadDataRefCB, NULL, // Float accessors
                               ...
                               NULL, NULL,             // Float array accessors
                               NULL, NULL,             // Raw data accessors
                               (void *)&current_head_pitch,
                               (void *)&current_head_pitch); // Refcons not used

  head_roll_out =
      XPLMRegisterDataAccessor("linuxtrack/pilots_head_roll",
                               xplmType_Float,         // The types we support
                               0,                      // Writable
                               NULL, NULL,             // Integer accessors
                               GetHeadDataRefCB, NULL, // Float accessors
                               ...
                               NULL, NULL,             // Float array accessors
                               NULL, NULL,             // Raw data accessors
                               (void *)&current_head_roll,
                               (void *)&current_head_roll); // Refcons not used
  ...
  enable_view_control = XPLMRegisterDataAccessor(
      ...
      NULL, NULL,                         // Float array accessors
      NULL, NULL,                         // Raw data accessors
      (void *)&head_control_enable,
      (void *)&head_control_enable); // Refcons not used
...
static int msgBoxHandler(XPWidgetMessage inMessage, XPWidgetID inWidget,
...
static void messageBox(const char *msgBoxTitle, const char *message) {
  ...
  int max_width = 50;
  int title_width = XPLMMeasureString(xplmFont_Proportional, msgBoxTitle,
                                      strlen(msgBoxTitle));
  ...
  for (i = 0; i < len; ++i) {
    if (msg_copy[i] == '\0') {
      ...
      lines[num_lines].width =
          XPLMMeasureString(xplmFont_Proportional, head, strlen(head));
  ...
  for (i = 0; i < num_lines; ++i) {
    /*XPWidgetID text = */ (void)XPCreateWidget(
        x + 20, ptr, x + 20 + lines[i].width, ptr - 15, 1, lines[i].text, 0,
        msgBox, xpWidgetClass_Caption);
    ptr -= 20;
  }
  ptr -= 10;
  /*XPWidgetID closeButton = */ XPCreateWidget(x + 30, ptr, x2 - 30, ptr - 15,
                                               1, "Close", 0, msgBox,
