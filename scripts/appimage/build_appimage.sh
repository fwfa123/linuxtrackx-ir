...
# LinuxTrack AppImage Builder
# Purpose: Create AppImage package with bundled dependencies and Wine bridge support
# Date: January 2025
# Status: Development
...
create_appdir_structure() {
    ...
    mkdir -p "$APPIMAGE_DIR/bin"
    mkdir -p "$APPIMAGE_DIR/lib"
    mkdir -p "$APPIMAGE_DIR/lib/linuxtrack"
    mkdir -p "$APPIMAGE_DIR/share"
    mkdir -p "$APPIMAGE_DIR/share/applications"
    mkdir -p "$APPIMAGE_DIR/share/icons"
    mkdir -p "$APPIMAGE_DIR/share/icons/hicolor/48x48/apps"
    mkdir -p "$APPIMAGE_DIR/share/icons/hicolor/scalable/apps"
    mkdir -p "$APPIMAGE_DIR/share/linuxtrack"
...
#!/bin/bash
# AppRun - LinuxTrack AppImage launcher

# Get the directory where the AppImage is mounted
APPDIR="$(dirname "$(readlink -f "$0")")"

# Set up environment
export LD_LIBRARY_PATH="$APPDIR/lib:$APPDIR/lib/linuxtrack:$LD_LIBRARY_PATH"
export QT_PLUGIN_PATH="$APPDIR/lib/qt5/plugins"
export QT_QPA_PLATFORM_PLUGIN_PATH="$APPDIR/lib/qt5/plugins/platforms"
...
EOF
    ...
    cp "$APPIMAGE_DIR/linuxtrack.desktop" "$APPIMAGE_DIR/share/applications/"
...
copy_icons() {
    ...
    if file_exists "$PROJECT_ROOT/src/linuxtrack.png"; then
        ...
        cp "$PROJECT_ROOT/src/linuxtrack.png" "$APPIMAGE_DIR/share/icons/hicolor/48x48/apps/"
        print_status "Copied PNG icon"
    fi
    
    if file_exists "$PROJECT_ROOT/src/linuxtrack.svg"; then
        cp "$PROJECT_ROOT/src/linuxtrack.svg" "$APPIMAGE_DIR/share/icons/hicolor/scalable/apps/"
...
bundle_dependencies() {
    ...
    # Bundle additional libraries manually
...
create_appimage() {
    ...
    print_success "AppImage created: ${APP_NAME}-x86_64.AppImage"
    return 0
}
...
show_help() {
    echo "LinuxTrack AppImage Builder"
    echo ""
...
main() {
    ...
    print_status "Starting LinuxTrack AppImage build..."
    ...
    print_success "LinuxTrack AppImage build completed successfully!"
